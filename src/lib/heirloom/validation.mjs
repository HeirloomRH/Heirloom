export function validateAllocations(amount, allocations) {
  if (!Number.isFinite(amount) || amount < 1 || amount > 100000000)
    return "Enter a demo amount between $1 and $100,000,000.";
  if (
    !allocations.length ||
    allocations.some(
      (a) => !Number.isFinite(a.weight) || a.weight <= 0 || a.weight > 100,
    )
  )
    return "Each selected asset needs an allocation greater than 0%.";
  if (Math.abs(allocations.reduce((s, a) => s + a.weight, 0) - 100) > 0.001)
    return "Your portfolio allocations must total 100%.";
  return "";
}
export function validateSchedule(
  schedule,
  today = new Date().toISOString().slice(0, 10),
) {
  if (!schedule.length) return "Add at least one release date.";
  if (
    schedule.some(
      (s) =>
        !/^\d{4}-\d{2}-\d{2}$/.test(s.date) ||
        !Number.isFinite(Date.parse(s.date)) ||
        new Date(s.date).toISOString().slice(0, 10) !== s.date ||
        s.date <= today,
    )
  )
    return "Choose valid future dates for every release.";
  if (schedule.some((s, i) => i > 0 && s.date <= schedule[i - 1].date))
    return "Release dates must be in chronological order, with no duplicates.";
  if (
    schedule.some(
      (s) => !Number.isFinite(s.percent) || s.percent <= 0 || s.percent > 100,
    )
  )
    return "Each release must be greater than 0% and no more than 100%.";
  if (Math.abs(schedule.reduce((n, s) => n + s.percent, 0) - 100) > 0.001)
    return "Your release percentages must total 100%.";
  return "";
}
export function validAddress(address) {
  return /^0x[a-fA-F0-9]{40}$/.test(address) && !/^0x0{40}$/i.test(address);
}
